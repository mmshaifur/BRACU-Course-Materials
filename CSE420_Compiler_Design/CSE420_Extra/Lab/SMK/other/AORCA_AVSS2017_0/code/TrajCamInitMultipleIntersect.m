...
% Brief: This script generates the trjectories with targets interecting at the same time
% Author: Yiming Wang - yiming.wang@qmul.ac.uk OR wangyimingkaren@gmail.com
% No permission necessary for non-commercial use
% Date: 27/08/2017

%% This file automatically generate the trjectories with a couple of targets interecting with varying angle with a constant velocity
...
for Nt = minNt:maxNt
    ...
    for index = 1:runIndex
        ...
        uniform_gap = 2*pi/(Nt+1);
        for i = 1:Nt
            disp('Number of targets:');
            disp(Nt)
            if i == 1
                angle(1) = 0;
            else
                generateGap = 1;
                while generateGap
                    angle_gap = uniform_gap*(1 + max(min(0.5*randn(1),0.5),-0.5));
                    criterion_upperlimit = (angle(i-1)+angle_gap)<2*pi;
                    criterion_lowerlimit = (angle(i-1)+angle_gap)>0;
                    criterion_angle_gap = abs(angle_gap-uniform_gap)<0.5*uniform_gap;
                    %criterion_angle_gap = 1;
                    if criterion_upperlimit&&criterion_lowerlimit&&criterion_angle_gap
                        generateGap = 0;
                    else
                        generateGap = 1;
                    end
                end
                angle(i) = angle(i-1)+ angle_gap;
            end
        ...
        if want2save
            save(['Setup/MobileCameras_MultiIntersect_' num2str(Nt) '_' num2str(ConstantVelo) '_' num2str(FixedDirection) '_' num2str(index)],'camLocM','camNumM','FOVM');
        end
        ...
        if save2figure
            imageFileName = ['RawImage/Setup/setup_MultiIntersect_' num2str(Nt) '_' num2str(ConstantVelo) '_' num2str(FixedDirection) '_' num2str(index)];
